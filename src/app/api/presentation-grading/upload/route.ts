...
import {
  ...
  PRESENTATION_AUDIO_MIME_TYPES,
  PRESENTATION_SLIDES_MAX_BYTES,
  PRESENTATION_SLIDES_MIME_TYPES,
} from "@/lib/presentation-grading";
import { logger } from "@/lib/logger";

const ClientPayloadSchema = z.object({
  kind: z.enum(["audio", "slides"]),
...
/** Blob upload tokens for presentation-grading media (staff only). */
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
        ...
        if (!allowedTypes.includes(payload.contentType)) {
          throw new Error(
            isAudio
              ...
              : "Slides must be a PDF or PPTX file"
          );
        }
    ...
    logger.warn("Upload token rejected", {
      route: "/api/presentation-grading/upload",
      error: message,
    });
