...
import type { FeedbackDraft, FeedbackEmailStatus } from "@/lib/feedback-email";

interface Props {
  kind: HumanGradingKind;
  jobId: string;
  status: FeedbackEmailStatus;
  /** Builds the editable draft, signed with the current user's name. */
  draftFor: (senderName: string) => FeedbackDraft;
  onChanged: () => void;
}

/** Lets staff forward the AI feedback to the student, showing when it last went out. */
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 space-y-0 pb-3">
        ...
        <Button size="sm" variant={sentBefore ? "outline" : "default"} onClick={() => setOpen(true)}>
      ...
      <CardContent className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
        <p>
          {sentBefore
            ...
            : "Not sent yet. Opens an editable draft of the AI feedback."}
        </p>
        <p className="text-xs">
          {status.studentEmail
