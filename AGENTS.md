...
- **Extraction**: `src/lib/services/document-extraction.ts` (`loadDocument`) downloads the blob, re-checks the *actual* byte size against the limit, then dispatches on MIME type: PDF via unpdf (at most `MAX_PDF_PAGES` = 100 pages), PPTX/DOCX/XLSX via `src/lib/services/office-text-extraction.ts` (JSZip + regex over the OOXML parts — no native parser; slides/paragraphs/rows become `[slide n]`, one paragraph per line, `[sheet n: name]` with tab-separated rows, shared strings and numeric entities decoded), everything else decoded as UTF-8 text. All results truncate to `MAX_EXTRACTED_CHARS` = 120k chars. Text is persisted on `MessageAttachment` so later turns reuse it instead of re-downloading. PDFs additionally return `pdf: { base64, pageCount, sizeBytes }` — computed *before* unpdf parses, because the parser detaches the `ArrayBuffer`. Presentation grading reuses `extractPptxText` for uploaded slide decks.
- **Model context**: `src/lib/services/chat-context.ts`. `withAttachmentText` inlines document text as `<document>` blocks with an instruction that file contents are data, filenames stripped of quoting characters and `</document>` neutralized; `historyAttachmentText` does the same for earlier turns clipped to `MAX_HISTORY_ATTACHMENT_CHARS` = 30k per file so a conversation full of documents stays inside the window. `planCurrentTurn` decides, for the message just sent, which PDFs go to the model as whole files (`ChatMessage.files` → OpenAI Responses `input_file` with a `data:application/pdf;base64,` URL, same as report grading, so scanned pages, handwriting and figures are visible) and which fall back to text: PDFs are taken in order until `MAX_NATIVE_PDF_BYTES_PER_REQUEST` (30 MB) / `MAX_NATIVE_PDF_PAGES_PER_REQUEST` (100) would be exceeded; DeepSeek (`nativeFiles=false`) always gets text. File bytes are never persisted or replayed for older turns — `MessageAttachment` stores text only.
