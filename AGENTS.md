...
- **Rubric**: `PresentationRubric` rows are append-only shared versions (seeded from `src/lib/default-presentation-rubric.ts`, written for the Fall 2026 format: one student, one Question Bank problem, ≤3:00 recorded video + ~3 min live Q&A, Track A "One Equation, One Model" / Track B "Counterfactual Physics Lab", same problem reused for the 2–3 page individual report — no groups, posters or AI/no-AI presentation conditions); each job pins the version it was graded with, so rubric edits never change existing results. The editor has a version-history panel (`GET /rubric/history`, latest 30 versions) — "Load into editor" puts an old version in the draft; restoring is just saving it as a new version.
