...
- **TA help** (`HowToGradeCard.tsx`, top of the grade tab): five numbered steps (drop zip → check fields → Start grading → read AI result, optional own scores → CSV export) plus an FAQ (no slides, unmatched ID, WMV first-run download, >3:30/no audio, finding a student by group/date). Open by default; Hide/Show persists in `localStorage` (`presentation-grading-help-collapsed` = `"1"`). Keep its wording in sync when button labels or limits change.
...
- **Purpose**: staff submit a written report / lecture notes (uploaded PDF or pasted text, exactly one of the two) and the AI returns a summary, referenced comments, and a per-criterion score (0–10 with weight and reason) for each rubric pillar; the TA/professor gives the final grade in person.
...
- **Rubric**: `ReportRubric` rows are append-only shared versions seeded from `src/lib/default-report-rubric.ts` (seven weighted pillars from the professor's suggested rubric); each job pins the version it was graded with. `GET/PUT /api/report-grading/rubric` + `GET .../rubric/history` mirror the presentation rubric flow.
...
- **Code**: service `src/lib/services/report-grading-service.ts`, shared constants/parsers `src/lib/report-grading.ts`, hooks `src/hooks/useReportGrading.ts`, API `src/app/api/report-grading/`, UI `src/app/(main)/report-grading/` + `src/components/report-grading/`, sidebar item "Report AI".
- **Batch upload & student IDs**: the form accepts multiple PDFs (≤40 per batch, `REPORT_BATCH_MAX_FILES`), creating one job per file with the title from the filename stem. Each job has a nullable `studentId` (≤50 chars, `REPORT_STUDENT_ID_MAX_CHARS`), auto-parsed from the filename by `studentIdFromFilename` (longest run of 5–15 digits, tolerates surrounding text); missing IDs show an amber warning and are editable in the batch list (`ReportBatchFileList.tsx`) or via the job's Edit dialog. `studentId` is searchable and shown in the list/detail.
- **CSV export (both grading tools)**: job lists have per-row checkboxes + Select all/Clear + "Export CSV", which fetches each selected job's detail and downloads a BOM-prefixed CSV (`src/lib/grading-csv.ts`, selection/export hooks `src/hooks/useGradingCsvExport.ts`). Report CSV: student ID, title, per-criterion AI score + human score + AI reason, AI and human weighted totals, human-grading audit columns, summary (blank cells for ungraded/legacy jobs). Presentation CSV: identifying fields + per-scorecard-category AI and human scores, both totals and the audit columns only — no questions/comments. `PresentationGradingJob.studentIds` (≤200 chars, `PRESENTATION_STUDENT_IDS_MAX_CHARS`, comma-separated) is enterable on creation, editable, searchable, and exported.
