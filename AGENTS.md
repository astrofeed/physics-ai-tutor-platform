...
- **TA help** (`HowToGradeCard.tsx`, top of the grade tab): five numbered steps (drop zip → check fields → Start grading → blind human scores then reveal AI → CSV export) plus an FAQ (no slides, unmatched ID, WMV first-run download, >3:30/no audio, finding a student by group/date). Open by default; Hide/Show persists in `localStorage` (`presentation-grading-help-collapsed` = `"1"`). Keep its wording in sync when button labels or limits change.
