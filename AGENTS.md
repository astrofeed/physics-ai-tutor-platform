...
**Cron job:** `GET /api/cron/send-scheduled-emails` — called every 5 minutes via [cron-job.org](https://cron-job.org). Queries `ScheduledEmail` where `status = PENDING AND scheduledAt <= now()`, sends emails, optionally creates in-app notifications, updates status, creates audit logs. Protected by `CRON_SECRET`. A schedule whose every recipient failed (e.g. Gmail not configured) is marked `FAILED` with the first error, never `SENT`; partial failures stay `SENT` with the failed count in `error`.

**Email never fails silently.** `sendEmail()` (`src/lib/email.ts`) throws `EmailNotConfiguredError` when `GMAIL_USER`/`GMAIL_APP_PASSWORD` are unset and rethrows transport errors — there is no log-and-return mode, so no caller can count an unsent message as sent. `sendBulkEmails()` reports rejections as `failedCount`/`errors` (and logs them); `POST /api/admin/email` answers 502 and writes no `bulk_email_sent` audit row when `sentCount === 0`, and `NotifyUsersDialog` shows a warning toast for partial failures. The spam guard still bans the user when the suspension email fails, but only records an Email Records row for a real send. Auth flows (`forgot-password`, `verify-email`, registration) keep their generic anti-enumeration responses and log the delivery error instead.
...
### Chat Attachments (images + PDF/PPTX/DOCX/XLSX/CSV/Markdown/text)

- **Limits** live only in `src/lib/chat-attachments.ts` and are shared by client pre-flight and server enforcement: 5 attachments per message; 5 MB per image, 10 MB per PDF/PPTX/DOCX/XLSX (`MAX_DOCUMENT_BYTES`), 1 MB per `.md`/`.txt`/`.csv`; 60 image uploads/hour; 30 documents and 150 MB of documents per rolling day (images deliberately excluded from the daily budget). Types are classified server-side from MIME type *and* extension, because Markdown/CSV often arrive with an empty or wrong MIME type; for `.pptx/.docx/.xlsx/.csv` the extension wins outright (browsers report Office files as `application/zip` or `application/vnd.ms-excel`). `SUPPORTED_ATTACHMENTS_LABEL` is the single source for every "what can I attach" string (picker title, drop overlay, client and server errors) — update it, not the call sites, when adding a type.
...
- **Blob pathnames**: every client-token route (`/api/upload/client`, `/api/report-grading/upload`, `/api/presentation-grading/upload`) returns `addRandomSuffix: true` from `onBeforeGenerateToken`. `@vercel/blob` ≥1.0 no longer suffixes by default and refuses to overwrite an existing pathname, so without it the second upload of any reused filename (`image.png`, a re-submitted report) failed with "This blob already exists". `useChatAttachments` surfaces the Blob SDK's own message when the token route would have issued a token.
- **Extraction**: `src/lib/services/document-extraction.ts` downloads the blob, re-checks the *actual* byte size against the limit, then dispatches on MIME type: PDF via unpdf (at most 30 pages), PPTX/DOCX/XLSX via `src/lib/services/office-text-extraction.ts` (JSZip + regex over the OOXML parts — no native parser; slides/paragraphs/rows become `[slide n]`, one paragraph per line, `[sheet n: name]` with tab-separated rows, shared strings and numeric entities decoded), everything else decoded as UTF-8 text. All results truncate to 30k chars. Text is persisted on `MessageAttachment` so later turns reuse it instead of re-downloading. Presentation grading reuses `extractPptxText` for uploaded slide decks.
...
- **Client**: `src/hooks/use-chat-attachments.ts` owns staging, validation, previews and upload; `ChatInput` renders image previews plus document chips, `MessageDocuments` renders them in the transcript. Validation errors are explicit: unsupported types say the file is not supported (never fail silently), and oversized files report the actual size next to the limit.
- **Drag & drop**: `src/hooks/use-file-drop.ts` turns a container into a drop target (drag-depth counter so child dragenter/dragleave don't flicker) and `FileDropOverlay` shows the visible target. Both the chat page main column (`ChatPageClient`) and `SideChatPanel` accept dropped files, routed through the same `selectFiles` validation as the picker.

### Presentation AI Pre-Grading (staff only)

- **Purpose**: before live presentations, staff upload a student's video + slides and the AI drafts a Part I analysis (scorecard, physics error log, items to verify in person) and Part II feedback notes for the TA/professor (Socratic guiding questions, live Q&A questions each with the reason to ask it). Nothing is student-facing; the TA/professor gives the final grade in person.
- **Presenter lookup + record management**: `PresentationGradingJob.presenters` (free text, required in the UI) exists so a TA can find a student's result right after the talk — the list has a debounced search over `presenters`/`topic` (`q` param on `GET /jobs`). Topic/presenters are editable via `PATCH /jobs/[id]`; `DELETE /jobs/[id]` hard-deletes the row plus any remaining temp blobs (confirmed with `AlertDialog` in `JobActions.tsx`).
...
- **Structured output**: grading uses OpenAI structured outputs (`zodTextFormat(PresentationEvaluationSchema)`), stored as JSON in `summaryJson`; `partIOutput`/`partIIOutput` are legacy markdown fields kept for jobs graded before the switch. The UI (`JobResult.tsx`) renders `StructuredResult` from `parseEvaluation(summaryJson)` and falls back to `LegacyResult.tsx` (markdown section parsing + the old tabs) for old jobs.
- **Result page layout** (`src/components/presentation-grading/`): the structured result is ordered for a grader reading top-down — `ScoreCard.tsx` (total + summary, then every rubric category with a score bar, `provisional` tag and justification; "Copy analysis"), `ChecksCard.tsx` ("Check before grading": teaching-team flags in amber with confidence, physics errors as numbered red cards with Check/Correct/Ask rows, required elements as a two-column present/weak/missing list with `ReferenceChip` timestamps), `FeedbackCards.tsx` (`LiveQaCard`: questions to ask + verify-in-person; `StudentFeedbackCard`: strengths, Socratic questions, report advice, topic suggestions; "Copy feedback"), then a collapsed "Source material" section (transcript / slides text as toggle buttons, raw text in a scrolling `max-h-[32rem]` block). Cards render nothing when their evaluation arrays are empty. The human `HumanScoreCard` from `PresentationGradedView.tsx` sits above this stack and is unaffected. `CopyButton.tsx` is the shared clipboard button.
- **Pipeline**: the browser extracts a 16 kHz mono WAV from the video (`src/lib/extract-audio.ts` — the video never leaves the TA's machine): Web Audio `decodeAudioData` first (MP4/MOV/WebM); when the browser cannot decode the container (WMV/ASF, AVI, MKV, …) it falls back to ffmpeg.wasm (`@ffmpeg/ffmpeg` + `@ffmpeg/util`, single-threaded `@ffmpeg/core@0.12.10` fetched from unpkg via `toBlobURL` on first use — no COOP/COEP headers needed; the instance is cached per page) with `-vn -ac 1 -ar 16000 -c:a pcm_s16le`, then the resulting WAV goes through the same Web Audio path. The hook reports the `transcoding` phase so `NewJobForm` can warn about the ~30 MB one-time download. It then uploads audio + slides via `/api/presentation-grading/upload` (staff-only Blob token route). `POST /jobs/[id]/process` (maxDuration 800) transcribes with `gpt-4o-mini-transcribe`, reads slides (PDF passed to the model as a file for vision; PPTX text extracted with jszip), grades with `gpt-5.6-luna` at the job's reasoning effort (`high` default, `xhigh` optional), then **deletes both blobs** — no media is retained. Jobs run independently; the client fire-and-forgets the process call, so closing the tab does not stop a job. Stale in-progress jobs (>15 min) can be taken over; FAILED jobs are retryable while their media still exists.
- **Rubric**: `PresentationRubric` rows are append-only shared versions (seeded from `src/lib/default-presentation-rubric.ts`, written for the Fall 2026 format: one student, one Question Bank problem, ≤3:00 recorded video + ~3 min live Q&A, Track A "One Equation, One Model" / Track B "Counterfactual Physics Lab", same problem reused for the 2–3 page individual report — no groups, posters or AI/no-AI presentation conditions). Its SCORING CALIBRATION section is deliberately lenient: every category starts in the 80–89% band and only drops for a cited error, 90+ is the normal result for correct well-checked work, one error is deducted once, wording imprecision and brevity are not deductions, and provisional categories default to the solid band — the earlier "typical presentation lands in the 70s / give the lower score" wording produced scores the teaching team found too harsh; the legacy `PresentationGradingJob.condition` column is unused (no form field, API input, CSV column or prompt line) and kept only to avoid a data-loss migration; each job pins the version it was graded with, so rubric edits never change existing results. The editor has a version-history panel (`GET /rubric/history`, latest 30 versions) — "Load into editor" puts an old version in the draft; restoring is just saving it as a new version.
...
- **Report topic suggestions**: `PresentationEvaluationSchema.topicSuggestions` (`TopicSuggestionsSchema`, nullable) carries a verdict (`revise` when the project has substantive problems — options extend from fixing them; `extend` when it is strong — three more advanced related directions), a short assessment, and exactly three options (title/direction/rationale, enforced by the prompt). `parseEvaluation` injects `topicSuggestions: null` for legacy stored JSON so old jobs keep rendering; `StudentFeedbackCard` shows the section only when present and `job-format.ts` includes it in copied notes.
- **No slides**: slides are optional. When a job has neither slides text nor a slides PDF, `buildGradingInput` appends `NO_SLIDES_NOTE` telling the model to be measured: open the summary by saying slides were unavailable, judge slide/figure criteria from the transcript only with tentative wording, give provisional (not penalised) scores where a criterion needs the slides, and add a "slides not submitted — inspect live" item to `verifyInPerson`. The job detail page shows `no slides submitted` in the header and an amber "based on the spoken transcript alone — treat slide-related scores as provisional" notice above the DONE result (both keyed on the persisted `slidesFilename`, which survives blob deletion).
- **eeClass ZIP import** (`src/lib/submission-zip.ts`, `src/components/presentation-grading/SubmissionImport.tsx`): the `ArchiveDropZone` at the top of `NewJobForm` ("Drop eeClass exports (.zip) here" — the primary way in, drag & drop or click, `multiple`, hidden input reset after every pick so the same zip can be dropped twice; non-zip drops get a toast) reads each archive **in the browser with jszip — the ZIP itself is never uploaded**, so a 25–30 MB export is fine; only the extracted video (≤500 MB / ≤3:30) and slides (≤30 MB) go through the normal validation. `parseSubmissionZip` skips `__MACOSX/`, dotfiles, `._*` and eeClass's `content.html`, groups media by student folder (the outermost folder whose name contains a student ID, so a wrapper folder around a bulk export is ignored; root files fall back to the archive name), takes the first video (`mp4/mov/m4v/webm/wmv/avi/mkv`) and first slides file (`pdf/pptx`) per folder, and parses `109062362 (林鍵鋒)` into `studentId` + `studentName` (`studentNameFromLabel`). One package → `NewJobForm.applySubmission` fills student ID, presenter, topic, video, slides (source switched to video). Several zips are parsed one after another (`parseArchives`: unreadable/empty ones toast and are skipped) and their packages merged. Several packages → an editable queue (name/topic per row; rows without a video are marked `no-video`) submitted **sequentially** through the same `useSubmitPresentationJob.submit`, so every batch row passes the same client + API validation as a manual job. Topic comes from the roster when the student ID matches, else from the video filename (toast tells the TA which). `FilePicker.tsx` is the shared file-chip picker extracted from `NewJobForm` to keep it under the line limit.
- **TA help** (`HowToGradeCard.tsx`, top of the grade tab): five numbered steps (drop zip → check fields → Start grading → read AI result, optional own scores → CSV export) plus an FAQ (no slides, unmatched ID, WMV first-run download, >3:30/no audio, finding a student by group/date). Rendered by the shared `HowToCard` (`src/components/grading/HowToCard.tsx`: title + steps + FAQ, open by default, Hide/Show persisted under the given `localStorage` key — `presentation-grading-help-collapsed` here, `report-grading-help-collapsed` for `ReportHowToCard`). Keep the wording in sync when button labels or limits change.
- **Sign-up sheet roster** (`PresentationRoster` + `PresentationRosterEntry`, migration `20260912000000_presentation_roster`): the course sheet (`DEFAULT_ROSTER_SHEET_URL` in `src/lib/presentation-roster.ts`) is imported automatically the first time a staff member loads the grade tab, and a roster imported more than an hour ago (`ROSTER_REFRESH_MS`) is re-imported from its own `sourceUrl` on the next load so late sign-ups, emails and report topics appear without anyone pressing Refresh (`GET /roster` → `getOrImportDefaultRoster`; a failed import is returned as `importError` alongside the previous roster and shown in `RosterCard`, never thrown — the roster only changes when the fetch and parse succeed), so TAs normally never paste anything. `GET /jobs/[id]` for both grading tools also calls `refreshRosterIfStale` first, so a result page opened directly (where the student email and English name come from) sees a current roster too; there a failed import is only logged and not retried for 5 minutes (`FAILED_REFRESH_RETRY_MS`, per server instance) so a dead sheet cannot add a 15 s fetch to every result read. This also means new roster columns (e.g. Email) reach production without a manual re-import. `RosterCard` shows the current sheet link prefilled with a **Refresh** button (re-import after new sign-ups); editing the link turns it into **Import this sheet** → `PUT /api/presentation-grading/roster` (Zod, staff-only) → `importRosterFromSheet` in `src/lib/services/presentation-roster-service.ts` rewrites the URL to that sheet's CSV export (`googleSheetCsvExportUrl` — only `https://docs.google.com/spreadsheets/…` is accepted; the sheet must be shared "anyone with the link can view"), fetches with a 15 s timeout and 2 MB cap, parses with the quoted-field CSV parser in `src/lib/presentation-roster.ts` (`parseRosterCsv` locates the header row by column names — Student ID / Chinese name / English name / Topics / date — carries the group label and date down merged cells, skips rows without a plausible student ID, dedupes), and replaces the previous roster in one transaction (only one roster exists at a time). `GET /roster` returns the summary (entry count, with-topic count, importer, time); `GET /roster/[studentId]` (`^\d{5,15}$`) returns `{studentId, name, englishName, topic, groupLabel, presentationDate}` and powers both the ZIP import and the student-ID field's `onBlur` auto-fill of empty presenter/topic fields (`lookupRosterStudent` in `src/hooks/usePresentationRoster.ts`). Verified against the Fall 2026 sheet: 176 students, 61 with a topic. **English name / group / date on jobs**: `PresentationJobSummary.englishName` / `groupLabel` / `presentationDate` are *not* stored on the job — `scheduleForJobs` in the grading service resolves them at read time from the roster (`RosterSchedule` from `rosterScheduleByStudentId`) for the job's first rostered student ID, so a re-import updates every job; they show in the job list meta line, the detail header ("presents 9/15") and as "English name" / "Group" / "Presentation date" CSV columns, and the list search (`jobSearchFilter` in the grading service) treats a query that `parseRosterSearch` reads as a group ("1", "Group 1", "第1組") or a date ("9/15") as *exactly* that group/date — `rosterStudentIdsMatching` compares the group number / month+day, so "Group 1" no longer pulls in Groups 10–19 — and lists only those students' jobs; other text is a substring match on topic / presenters / studentIds. `usePresentationJobs` drops out-of-order list responses (request sequence ref) so a slow earlier search can't overwrite a later one.
- **Group menu on the results lists** (report + presentation): `JobListFilters` (`src/components/presentation-grading/JobListFilters.tsx`, shared by `JobList` and `ReportJobList` via `JobListFilterProps`) renders an "All groups / Group 1 … Group N" `Select` beside the search box. The options are `RosterSummary.groupNumbers` (distinct numbers parsed from the sheet's group labels by `rosterGroupNumbers`, ascending — 1–23 on the Fall 2026 sheet), so the pages now own `usePresentationRoster()` and pass its `RosterState` into `RosterCard` and the group list into the job list; the menu is hidden while the roster hasn't loaded / has no groups. The choice lives in `useReportJobs` / `usePresentationJobs` (`group`, `setGroup` resets to page 1) and is sent as `?group=N`; both `GET …/jobs` routes parse `q` + `group` with `parseJobListFilter` (`src/lib/presentation-roster.ts`, group must be an integer 1–999, other values ignored) into a `JobListFilter` that `listReportJobs` / `listPresentationJobs` turn into an `AND` of the text condition (`jobSearchFilter`) and the group condition (`rosterJobFilter({kind:"group"})` → `rosterStudentIdsMatching(RosterSearch)`, exact group number, so Group 1 never lists Groups 10–19), so text + group narrow together and pagination/total reflect the filtered set. `rosterStudentIdsMatching` now takes a parsed `RosterSearch` rather than the raw query string.
...
### Written-Report AI Grading (staff only)

- **Purpose**: staff submit written reports (uploaded PDFs / eeClass zips, or pasted text for a single report — exactly one of `reportBlobUrl`/`reportText` per job) and the AI returns a summary, referenced comments, and a per-criterion score (0–10 with weight and reason); the TA/professor gives the final grade in person.
- **Topic + assigned question**: every report belongs to the student's presentation topic (the job `title`, filled from the sign-up sheet's `Topics` column) and may carry an **additional report question** from the sheet's `Report Topic` column, stored on the job as `ReportGradingJob.assignedQuestion` (`String? @db.Text`, ≤`REPORT_ASSIGNED_QUESTION_MAX_CHARS` = 2000, migration `20260913000000_report_topic_and_assigned_question`, which also adds `PresentationRosterEntry.reportTopic`). It is copied onto the job at creation (assignment context must not change if the sheet is edited later) and sent to the model as `Presentation topic (report title)` / `Additional report question` lines in REPORT INFORMATION; the guard tells the model to judge topic coverage AND whether the question was answered, and never to penalise a report when no question was set. Shown on the detail page, in the batch list (editable) and the CSV; UI copy calls it the **report question** (sheet column `Report Topic`) and the title the **presentation topic** (sheet column `Topics`). `parseRosterCsv` claims `Report Topic` (`/report\s*topic|報告題目/`) before the generic `topic` pattern and each header column is claimed once.
- **Result shape**: OpenAI structured outputs (`zodTextFormat(ReportEvaluationSchema)`) stored as a JSON string in `ReportGradingJob.resultJson`; parsed with `parseReportEvaluation`. Fields: `summary`, `comments[{reference, comment}]`, `criterionScores[{criterion, weightPercent, score, reason}] | null`. Jobs graded before criterion scores existed (which had `questions` instead) parse with `criterionScores: null` and render without the scores section.
- **Rubric**: `ReportRubric` rows are append-only shared versions seeded from `src/lib/default-report-rubric.ts`; each job pins the version it was graded with. `GET/PUT /api/report-grading/rubric` + `GET .../rubric/history` mirror the presentation rubric flow. The default has seven weighted criteria (topic coverage & assigned question 25, physics correctness & first principles 25, derivation completeness 15, figures/notation 10, limits/units/sanity checks 10, self-containment/sources 10, presentation/integrity 5) and a "Scoring calibration" section that is deliberately lenient like the presentation one: every criterion starts at 8/10, explicit bands (9–10 / 8 / 6–7 / 4–5 / 0–3), only cited evidence lowers a score, one error is deducted once, wording/brevity/elementary-but-correct maths are feedback not deductions, ties go to the higher band, and a competent report lands around 8–8.5. Criterion 1 has its own bands and grades topic coverage alone when no question was set. The production `ReportRubric` row is **not** rewritten by code — staff paste a new version in the editor when they want it.
- **Constraints**: PDF only, ≤30 MB (`REPORT_FILE_MAX_BYTES`); pasted text ≤120k chars (`REPORT_TEXT_MAX_CHARS`); reasoning effort `high`/`xhigh`; model `gpt-5.6-luna`. The temp report blob is deleted after processing; FAILED jobs are retryable while their source remains.
- **Security**: all routes `requireApiRole(STAFF_ROLES)`, Zod-validated with `.max()` bounds, blob URLs restricted to `*.blob.vercel-storage.com`, report text wrapped as untrusted data in the grading prompt, and job creation rate limited via `consumeActionRateLimit` (`report_grading_job`, 60/hour). Grading must be evidence-based: the guard forbids inventing content and frames integrity issues as evidence-based concerns.
- **Code**: service `src/lib/services/report-grading-service.ts` (jobs, rubric versions, listing/search, retry/delete) + `report-grading-ai.ts` (blob download, prompt building, the OpenAI structured-output call — split out to keep the service under the line limit), shared constants/parsers `src/lib/report-grading.ts`, hooks `src/hooks/useReportGrading.ts` (rubric, job list, sequential submit) + `src/hooks/useReportBatch.ts` (client batch assembly), API `src/app/api/report-grading/`, UI `src/app/(main)/report-grading/` + `src/components/report-grading/`, sidebar item "Report AI".
- **Batch upload (PDFs + eeClass zips)**: `ReportDropZone.tsx` at the top of `NewReportJobForm` accepts any mix of `.pdf` and `.zip` (drag & drop or click, `multiple`, input reset after each pick). `useReportBatch.addFiles` keeps PDFs as-is and opens zips **in the browser** with `parseReportZip` (`src/lib/submission-zip.ts`: every `.pdf` entry becomes one `ReportPackage`, labelled by its student folder or the archive name; `__MACOSX/`, dotfiles, `content.html` and all non-PDF files such as `index.html` are skipped; a folder with several PDFs yields several rows — the TA removes the non-report ones). Each row (`ReportBatchFile`: file, folder, studentId, studentName, title, assignedQuestion, groupLabel, presentationDate, rosterMatched) gets its student ID from the folder/filename (`studentIdFromFilename`), then every distinct ID is looked up once via `lookupRosterStudent` (`GET /api/presentation-grading/roster/[studentId]`, which now returns `reportTopic`) and `applyRoster` fills name, title (= sheet `Topics`, else filename stem), assignedQuestion (= sheet `Report Topic`, else blank), group and date. `ReportBatchFileList.tsx` shows every row with editable ID / name / title / question; editing the ID and blurring calls `refillFromRoster` (name is re-derived from the sheet, a hand-typed name is kept only until then). Rows without an ID or not in the sheet carry an amber triangle plus a hint line; matched rows show `Group · date`. `useSubmitReportJob.submit` posts the rows one after another (title/authors=name/studentId/assignedQuestion per row, blank → omitted), ≤40 per batch (`REPORT_BATCH_MAX_FILES`; the hook truncates and toasts). The page also mounts the shared `RosterCard` (same roster as presentations) and `ReportHowToCard` (steps + FAQ: what the assigned question is, unmatched ID, several PDFs in a folder, non-PDF reports, finding a group). Paste-text mode still uses the single title/authors fields.
- **English name / group / date on report jobs**: like presentations, `ReportJobSummary.englishName` / `groupLabel` / `presentationDate` are resolved at read time from the current roster by `studentId` (`scheduleForJobs` → `rosterScheduleByStudentId`), shown in the list rows and detail header, exported in the CSV ("English name" / "Group" / "Presentation date" columns), and searchable exactly (`jobSearchFilter`: `parseRosterSearch` → `rosterStudentIdsMatching`, so "Group 1" never matches Group 10; other queries stay `contains` over title/authors/studentId). Each job has a nullable `studentId` (≤50 chars, `REPORT_STUDENT_ID_MAX_CHARS`), editable via the job's Edit dialog.
- **CSV export (both grading tools)**: job lists have per-row checkboxes + Select all/Clear + "Export CSV", which fetches each selected job's detail and downloads a BOM-prefixed CSV (`src/lib/grading-csv.ts`, selection/export hooks `src/hooks/useGradingCsvExport.ts`). Report CSV: student ID, title, authors, English name, group, presentation date, report question, status, per-criterion AI score + human score + AI reason, AI weighted total and human total (entered directly or weighted from items), human-grading audit columns (incl. "Human scoring" = total only / per item), summary (blank cells for ungraded/legacy jobs). Presentation CSV: identifying fields + per-scorecard-category AI and human scores, both totals, the audit columns, then the AI's **report-topic suggestions** — "Report topic verdict" (revise/extend), "Report topic assessment" and one "Suggested report topic N" column per option (`title — direction`; the column count is the widest option list among the exported jobs, blank when `topicSuggestions` is null) — no questions/comments. `PresentationGradingJob.studentIds` (≤200 chars, `PRESENTATION_STUDENT_IDS_MAX_CHARS`, comma-separated) is enterable on creation, editable, searchable, and exported.
- **Human scores vs AI (both grading tools, for the AI-vs-human agreement study)**: `ReportHumanScore` (per rubric criterion) and `PresentationHumanScore` (per scorecard category) rows store the staff score as `Decimal`, unique per job+item, cascade-deleted with the job. Both job models carry `humanGradedAt` (first save, never moved by later edits), `humanGradedById` (last grader) and `aiRevealedAt` (first time staff opened the AI result; `revealReportAi`/`revealPresentationAi` are idempotent conditional `updateMany`s so it never moves). `gradedBlind()` in `src/lib/human-grading.ts` compares the two timestamps — `humanGradedAt <= aiRevealedAt` means the human graded blind. `PUT /jobs/[id]/human-scores` (`HumanScoresInputSchema`, a union of `{ scores }` (≤50 entries) or `{ total }`) is only valid for DONE jobs with a parsed AI result and rejects unknown/duplicate items and scores outside `[0, max]` (report: `REPORT_CRITERION_MAX_SCORE` = 10; presentation: the category's `maxPoints`) or a total above the scale (report 10, presentation = sum of `maxPoints`); `POST /jobs/[id]/reveal-ai` records the reveal. **Total-only grading** (professors grade between back-to-back talks): both job models carry `humanTotal Decimal?` (migration `20260915052912_human_total_score`); saving `{ total }` sets it and deletes the per-item rows, saving `{ scores }` clears it — the two modes are exclusive. `HumanGrading.total` exposes it; `humanTotalOf(human, fromItems)` returns the entered total or derives one from the items (`sumScores` for presentations, `weightedAverage` for reports) and is what the CSV "Human total" columns and the card's Total row use. The CSV audit block gained a "Human scoring" column (`total only` / `per item` / blank) so the analysis can separate the two. Service: `src/lib/services/human-grading-service.ts`; `getReportJob`/`getPresentationJob` return `human: HumanGrading` with the saved scores. UI: on a DONE job `ReportGradedView`/`PresentationGradedView` render the AI result immediately (no gate — graders asked for less friction) with the optional `HumanScoreCard` (shared, `src/components/grading/`, form split into `HumanScoreForm.tsx`, shared item types in `human-score-items.ts`) underneath; the form has a "Total only" / "Per item" toggle (defaults to Total only unless per-item scores were saved), shows the AI score/total beside each input and, once saved, a You / AI / Δ table with an emphasised Total row (the only row in total-only mode). The Total field is focused only when the grader clicked **Edit scores** (`focusOnOpen`); on page load the form must never `autoFocus`, because the card sits at the bottom of the result page and the browser would scroll straight past the AI result to it. The views pass `total: { max, aiScore }` and `totalFromItems`. `useHumanGrading(kind, jobId, aiRevealedAt, onChanged)` POSTs `reveal-ai` on mount when `aiRevealedAt` is null, so the timestamps still separate blind grades (only possible via the API/CSV, never the UI now) from AI-informed ones; failures are logged, not toasted. Legacy jobs without criterion scores just show the result. Migration `20260910154111_human_scores_for_grading_jobs`.

- **Email the AI feedback to the student (both grading tools)**: on a DONE job with a parsed AI result, `FeedbackEmailCard` (shared, `src/components/grading/`) sits between the AI result and the human scorecard; **Email student** opens `FeedbackEmailDialog`, an editable To / Subject / Message form. The draft comes from `GET /api/{report,presentation}-grading/jobs/[id]/feedback-email` (staff-only; `useFeedbackEmail().loadDraft`, spinner in the dialog until it arrives, toast + close on failure) → `draftReportFeedback`/`draftPresentationFeedback` in `src/lib/services/feedback-draft-service.ts` (404 unknown job, 409 no parsed AI result). The service asks `OPENAI_CHAT_MODEL` (low reasoning, `LETTER_GUARD`) to rewrite the grader-facing result as a short letter in the instructor's voice — presentation: summary + strengths woven into the assessment + suggested report topics as a closing prose paragraph; report: summary only — with no headings, bullets, scores, "the presenter", provisional/missing-slides remarks or LaTeX; the greeting (roster English name, fallback authors/presenters) and `Best regards, <name>` sign-off are added deterministically around the model's body. Without `OPENAI_API_KEY` or when the call fails (logged), the response falls back to the template builders in `src/lib/feedback-email.ts` (`reportFeedbackDraft`: greeting + summary; `presentationFeedbackDraft`: greeting + summary + numbered topic options, no section labels) and `rewritten: false`, which the dialog states in its description so staff know to edit. The sign-off uses the server-side *effective* user (`requireApiRole` → `getEffectiveSession`), so impersonation/E2E cookies sign as the acting grader, matching the audit row. Guiding questions, report advice and evidence-referenced comments are deliberately not in the email (professors asked for summary + suggested topics only). The recipient is prefilled from the sign-up sheet's **Email** column (`PresentationRosterEntry.email`, header `/e-?mail|信箱/`, lowercased, kept only if it looks like an address; surfaced on the job detail as `studentEmail` via `rosterScheduleByStudentId`) and stays editable because sheets have typos; no roster email → amber hint and an empty field. The form is mounted only once a draft has arrived so every opening starts from a freshly written draft. `POST /api/{report,presentation}-grading/jobs/[id]/feedback-email` (staff-only, `FeedbackEmailInputSchema`: email ≤200, subject ≤500, message ≤10,000) → `sendReportFeedback`/`sendPresentationFeedback` in `src/lib/services/feedback-email-service.ts`: 404 for unknown jobs, 503 when `sendEmail` throws `EmailNotConfiguredError` (Gmail not configured), 502 when the transport rejects; on success it stamps `feedbackSentAt`/`feedbackSentTo` on the job (shown on the card as "Sent to … on …" with a **Send again** button) and writes an `AuditLog` row `grading_feedback_emailed` in the same `details` shape as `bulk_email_sent` (`performedBy`/`performedByName`, `recipientIds` = the platform user id when the address belongs to an account else the raw address, `recipientCount: 1`, `subject`, `message`, plus `jobKind`/`jobId`), so the admin **Email Records** page lists it as "Grading feedback sent by …" — `GET /api/admin/email-records` queries `EMAIL_SENT_AUDIT_ACTIONS` (`src/lib/constants.ts`) rather than only bulk sends. HTML comes from `gradingFeedbackEmail` in `src/lib/email-templates.ts` (escaped, `white-space: pre-wrap`, staff-name footer). Client hook `useFeedbackEmail(kind, jobId, onSent)` toasts and reloads the job. Migration `20260915120000_feedback_email`.
