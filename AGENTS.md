...
- **Auto-graded scores are editable**: the grading UI labels them "Auto-graded — edit the score to override" and the score input is never disabled. Any manual save writes `autoGraded: false` (`grading-service.ts`), so an override is distinguishable from a machine score. Bounds (`0 ≤ score ≤ question.points`) still apply.
