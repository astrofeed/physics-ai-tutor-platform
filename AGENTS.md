...
- **Report topic suggestions**: `PresentationEvaluationSchema.topicSuggestions` (`TopicSuggestionsSchema`, nullable) carries a verdict (`revise` when the project has substantive problems — options extend from fixing them; `extend` when it is strong — three more advanced related directions), a short assessment, and exactly three options (title/direction/rationale, enforced by the prompt). `parseEvaluation` injects `topicSuggestions: null` for legacy stored JSON so old jobs keep rendering; `EvaluationView` shows the section only when present and `job-format.ts` includes it in copied notes.
