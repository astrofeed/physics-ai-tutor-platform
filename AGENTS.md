...
- **No slides**: slides are optional. When a job has neither slides text nor a slides PDF, `buildGradingInput` appends `NO_SLIDES_NOTE` telling the model to judge slide/figure criteria from the transcript only and to add a "slides not submitted — inspect live" item to `verifyInPerson`; the job detail header shows `no slides submitted` (from the persisted `slidesFilename`, which survives blob deletion).
