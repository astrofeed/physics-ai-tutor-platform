...
- **Purpose**: staff submit a written report / lecture notes (uploaded PDF or pasted text, exactly one of the two) and the AI returns **only** a summary, referenced comments, and questions for the author — no numeric score; the TA/professor grades in person.
- **Result shape**: OpenAI structured outputs (`zodTextFormat(ReportEvaluationSchema)`) stored as a JSON string in `ReportGradingJob.resultJson`; parsed with `parseReportEvaluation`. Fields: `summary`, `comments[{reference, comment}]`, `questions[{question, reason}]`.
