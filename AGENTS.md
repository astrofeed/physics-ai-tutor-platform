...
- **A generated answer key is never verified against its own solution** and a wrong key still grades cleanly, so creating an assignment from generated problems warns the author (`VERIFY_KEYS_NOTICE`) to read every key before publishing. Prompting for it (the model is told to re-read its solution's final line) reduces but cannot remove this; there is deliberately no automated answer-vs-solution check.
